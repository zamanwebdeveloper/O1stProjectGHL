...
function modify_tract($hawb_no, $origin, $destination, $through, $awb_no, $date, $shipper, $receiver, $product, $status,$smobile,$semail,$saddress,$rmobile,$remail,$raddress,$edate,$agentname,$refname,$cLocation,$pcs,$weight)
{
	$sql="UPDATE shipment set origin='$origin', destination= '$destination', media='$through', AWB='$awb_no', pdate='$date', shipper='$shipper', receiver='$receiver', product='$product', status='$status', smobile='$smobile', semail='$semail', saddress='$saddress', rmobile='$rmobile', remail='$remail', raddress='$raddress', edate='$edate', agentname='$agentname', refname='$refname', cLocation='$cLocation', pcs='$pcs', weight='$weight' where HAWB='$hawb_no'";
	$res=mysql_query($sql) or die(mysql_error());
	if($res)
	{
		return ture;
